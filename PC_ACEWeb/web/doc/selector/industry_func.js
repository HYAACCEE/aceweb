...
var Industry = {
	...
	init : function(){
		...
		if (ind_flag_arr.length>0){
			...
			$curpdiv_.find('#btn_IndustryID').val(_str.substring(1));
			$curpdiv_.find('#IndustryID').val(_id.substring(1));
		}
	},
	Show : function(){
	...
	confirm : function(){
		...
		$curpdiv_.find('#btn_IndustryID').val(indStr);
		$curpdiv_.find('#IndustryID').val(ind_flag_arr);
		boxAlpha();
...
/* ****************************** 单选 ********************************* */
	...
	Chk2 : function(id){
		$curpdiv_.find('#btn_IndustryID_2').val(ind_a[id]);
		$curpdiv_.find('#IndustryID_2').val(id);
		boxAlpha();
	}
}
...
// 单选
function IndustrySelect_2(pdiv){
	$curpdiv_ = pdiv;
	var dragHtml ='<div id="IndustryAlpha">';		//行业
