...
var residency = {
	...
	Chk : function(id){
		if(residency_hukou_flag==0){
			$curpdiv_.find('#btn_residency').val(ja[id]);
			$curpdiv_.find('#residency').val(id);
		}else{
			$curpdiv_.find('#btn_hukou').val(ja[id]);
			$curpdiv_.find('#hukou').val(id);
		}
		$("#sublist").hide().empty();
		boxAlpha();
	}
}
function residencySelect(pdiv){
	$curpdiv_ = pdiv;
...
var jobArea = {
	...
	init : function(){
		...
		if (jobArea_Arr.length>0){
			...
			$curpdiv_.find('#btn_jobArea').val(_str.substring(1));
			$curpdiv_.find('#jobAreaID').val(_id.substring(1));
		}
	},
	Show : function(){
	...
	confirm : function(){
		...
		$curpdiv_.find('#btn_jobArea').val(areaStr);
		$curpdiv_.find('#jobAreaID').val(jobArea_Arr);
		boxAlpha();
		$('#jobAreSelected dd').empty();
	}
}

function jobAreaSelect(pdiv){
	$curpdiv_ = pdiv;
