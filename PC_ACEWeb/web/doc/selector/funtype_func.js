...
function funtypeSelect(){
	var dragHtml ='<div id="FuntypeAlpha">';			//职能类别
		dragHtml+='		<dl id="FuntypeSelected"><dt>已选择职业技能：</dt><dd></dd></dl>';
		dragHtml+='		<div id="FuntypeList"></div>';	//职能类别列表
		dragHtml+='</div>';
	$('#drag_h').html('<b>请选择职业技能（您最多能选择5项）</b><span onclick="Funtype.confirm()">确定</span>');
...
//赋值
function setFuntypeSelect(array){
	for(var i in fun_a){
		for(var j in array){
			if(fun_a[i] == array[j]){
				fun_flag_arr.push(i);
...
function funtypeSelect_2(){
	var dragHtml ='<div id="FuntypeAlpha">';			//职能类别
		dragHtml+='		<div id="FuntypeList"></div>';	//职能类别列表
		dragHtml+='</div>';
	$('#drag_h').html('<b>请选择职业技能</b><span onclick="boxAlpha()">关闭</span>');
	$('#drag_con').html(dragHtml);
	Funtype.Show2();
	boxAlpha();
	draglayer();
