...
var UPLOAD_PIC_MAXSIZE = 1*1024*1024;
var DEBUG = false;
var onlogFlag = true;
...
var URL_SPLITER_CID = "g=098";
function getUrlCid(){
  var url = location.href;
  if(url.indexOf(URL_SPLITER_CID)>0) return url.split(URL_SPLITER_CID)[1];
}
function getUrlTid(){
  var url = location.href;
  ...
  if(pos>0){
    ...
    return url.split("&"+URL_SPLITER_CID)[0];
  }
  return "";
}
function getUrlPid(){
...
function ACECommon_Submit(url, data, sFun, options){
  var $ap = new APost(url, data, sFun, options);
  $ap.Submit();
}
function ACECommon_FormSubmit(btn, url, data, sFun, formDiv, nullMsg){
  var $ap = new APost(url, data, sFun, {'div':formDiv, 'btn':btn, 'nullMsg':nullMsg});
  $ap.Submit();
}
APost.prototype.Submit = function(){
  ...
  if(uploads_num>0){
    ajaxFile_("MMMService.y?cmd=uploadPic", "{N:'"+uploads_num+"'}", function(html){
      html = html.split("`");
...
APost.prototype.doPost = function(){
  ...
  log("[ACECommon_FormSubmit] formParam="+$ap.formParam);
  log("[ACECommon_FormSubmit] formContent="+$ap.formContent);
