//v='com00000027'
var cmmv = (new Date()).getTime()+"42701";
var auid,mid,amid,power,userName,userFace,M0;
var noticeShow;
var PATH = '../';
var SP = '`', SP0 = '_SP0;', SP1 = '_SP1;', SP2 = '_SP2;';
var UPLOAD_FILE_MAXSIZE = 2*1024*1024;
var UPLOAD_PIC_MAXSIZE = 2*1024*1024;
var DEBUG = false;
var onlogFlag = true;

var mb = myBrowser();
var isChrome = "Chrome" == mb;
var isSafari = "Safari" == mb;
var isIE = "IE" == mb;
var isFirefox = "FF" == mb;
var isOpera = "Opera" == mb;
...
//www.ace-elite.com
//var cnzz_protocol = (("https:" == document.location.protocol) ? " https://" : " http://");document.write(unescape("%3Cspan id='cnzz_stat_icon_1261795902'%3E%3C/span%3E%3Cscript src='" + cnzz_protocol + "s95.cnzz.com/z_stat.php%3Fid%3D1261795902%26show%3Dpic1' type='text/javascript'%3E%3C/script%3E"));

var CID = null, PCID = null;
var URL_SPLITER_G = "g=098";
var URL_SPLITER_CID = URL_SPLITER_G+cmmv;
function getUrlCid(){
  var url = location.href;
  if(url.indexOf(URL_SPLITER_G)>0) return url.split(URL_SPLITER_G)[1].substring(cmmv.length);
}
function getUrlTid(){
  var url = location.href;
  var pos = url.indexOf("tid=");
  if(pos>0){
    var tmp = url.split("tid=");
    url = tmp[1];

    pos = url.indexOf("&pcid=");
    if(pos>=0){
      return url.split("&pcid=")[0];
    }
    
    return url.split("&"+URL_SPLITER_G)[0];
  }
  return "";
}
function getUrlPid(){
  var url = location.href;
  var pos = url.indexOf("pid=");
  if(pos>0){
    var tmp = url.split("pid=");
    url = tmp[1];
...
function getUrlPcid(){
  var url = location.href;
  var pos = url.indexOf("pcid=");
  if(pos>=0){
    var tmp = url.split("pcid=");
    url = tmp[1];
...
function getUrlHcid(){
  var url = location.href;
  var pos = url.indexOf("hcid=");
  if(pos>=0){
    var tmp = url.split("hcid=");
    url = tmp[1];
...
function JSONSTR(a){
  return a.replace(new RegExp(SP0, "g"), '"');
}
/**********************************************************/
...
$(function(){
  ...
  if($('#former')) $('#former').css("min-height", c_h+'px');
	...
	$(".hr_").click(function(){ onlog('点击我要招人'); go("../acehr/index.html"); });
	$(".hunter_").click(function(){ onlog('点击我是猎头'); go("../acehc/user_login.html"); });
...
function DataLine_InitFuns(pdiv){
  ...
  $dataline_.hover(
      function (){
        ...
        var isHome = $(this).find(".actions_").attr("data");
        var isCollect = $(this).find(".actions_").attr("data1");
        if(isHome == "1"){
        	if(isCollect=="0"){
		        $(this).find(".actions_").show();
		        $(this).find("#candidNumber_").hide();
		        $(this).find("#itemFooter_").hide();
        	}else{
		        $(this).find(".actions_").hide();
		        $(this).find("#candidNumber_").show();
		        $(this).find("#itemFooter_").show();
        	}
        }else{
	        $(this).find(".actions_").show();
        }
...
/****************** ACE 通用 展示HTML *************************/
function ACECommon_Data(url, data, sFun, errFun, options){
  options = options || {};
  var btn = options['btn'];
  var div = options['div'];
  var nullMsg = options['nullMsg'] || '0';
  var extdata = options['extdata'] || '';
  ACECommon_ShowData(url, data, div, nullMsg, sFun, errFun, extdata);
}
function ACECommon_ShowData(url, data, div, errFlag, sfun, errFun, extdata){
...
/****************** ACE 通用 提交form *************************/
function APost(url, data, sFun, options){
  this.url = url;
  this.data = data;
  this.sFun = sFun;
  this.options = options || {};
  this.btn = options['btn'];
  this.formdiv = options['div'];
  this.nullMsg = options['nullMsg'] || '1';
  this.autoCloseLoading = options['autoCloseLoading'] || 1;
  this.uploadFileNum = options['uploadFileNum'] || 0;
  this.formParam = "";
  this.formContent = "";
}
APost.prototype.Submit = function(){
  var $ap = this;
  
  if($ap.btn) {$ap.btn.attr("disabled", "disabled"); $ap.btn.addClass("btn-disabled");}
  var notnull = $ap.formDiv==null ? $(".notnull") : $ap.formDiv.find(".notnull");  
  var flag = true;
  if($ap.nullMsg==1){
  ...
  if(!flag){
    if($ap.autoCloseLoading == 1) util.UI.closeLoading();
    if($ap.btn) { $ap.btn.removeAttr("disabled"); $ap.btn.removeClass("btn-disabled");}
    return false;
  }
  
  var $vvf = $(".vvf");
  var uploads_num = $vvf.length;
  if(uploads_num>0){
    ajaxFile_("MM"+M0+"Service.y?cmd=uploadFile", "{N:'"+uploads_num+"'}", function(html){
      html = html.split("`");
      var $uploadFile = $(".uploadFile");
      for(var m=0;m<uploads_num;m++){
        if($uploadFile.eq(m)[0].files[0]) $vvf.eq(m).val(html[m]);
      }
      $ap.doPost();
    });
  }else{
...
APost.prototype.doPost = function(){
  var $ap = this;
  
  var vv = $ap.formDiv==null ? $(".vv") : $ap.formDiv.find(".vv");
  for(var i=0; i<vv.length; i++){
    $ap.formParam += SP + $(vv[i]).val().replace(new RegExp(SP, "g"), "").replace(/(^\s*)|(\s*$)/g, "");
  }
  $ap.formParam = $ap.formParam.substring(1);
  $ap.formParam = $ap.formParam.replace(new RegExp("'", "g"), "&#39;");
  vv = $ap.formDiv==null ? $(".vvc") : $ap.formDiv.find(".vvc");
  for(var i=0; i<vv.length; i++){
    $ap.formContent += SP + $(vv[i]).val().replace(new RegExp(SP, "g"), "").replace(/(^\s*)|(\s*$)/g, "");
  }
  $ap.formContent = $ap.formContent.substring(1);
  $ap.formContent = $ap.formContent.replace(new RegExp("'", "g"), "&#39;");
  
  log("[APost.doPost] formParam="+$ap.formParam);
  log("[APost.doPost] formContent="+$ap.formContent);
  if($ap.data && $ap.data.length>0) $ap.formParam = $ap.data.replace("}", ",formParam:'"+$ap.formParam+"', formContent:'"+$ap.formContent+"'}");
  else formParam = "{formParam:'"+$ap.formParam+"', formContent:'"+$ap.formContent+"'}";
  ajax_($ap.url, $ap.formParam, function(ret, html){
      if(ret==2) util.UI.err(html);
      if($ap.sFun) $ap.sFun(ret, html);
      if($ap.btn) { $ap.btn.removeAttr("disabled"); $ap.btn.removeClass("btn-disabled");}
  });
  if($ap.autoCloseLoading==$ap.autoCloseLoading) setTimeout("util.UI.closeLoading();", 20000); //设置10秒后释放
  if($ap.btn) setTimeout("$ap.btn.removeAttr('disabled'); $ap.btn.removeClass('btn-disabled');", 10000); //设置10秒后提交按钮 显示
}

function ACECommon_Submit(url, data, sFun, options){
  options = options || {};
  var btn = options['btn'];
  var div = options['div'];
  ...
  ACECommon_FormSubmit(btn, url, data, sFun, div, nullMsg, autoCloseLoading, uploadFileNum);
}
function ACECommon_FormSubmit($btn_, url, data, sFun, formDiv, nullMsg_, autoCloseLoading_, uploadFileNum){
  var nullMsg = nullMsg_ || 1;
  var autoCloseLoading = autoCloseLoading || 1;
  ...
  log("[ACECommon_FormSubmit] formParam="+formParam);
  log("[ACECommon_FormSubmit] formContent="+formContent);
...
function ajax_(url, data, sfun){
  ...
  ajax(PATH+'HIOnline.y?cmd=apply',
         ...
         function(html){
             ...
             var result = html.split("`USER=");
             if(result.length>1 && result[1].length>0){
               var user = JSON.parse(result[1]);
               reloadUserCookie(user);
             }
             var ret = result[0].substring(0,1);
             if(sfun) sfun(ret, result[0].substring(2));
        });
}
...
function onlog(a){
	if(onlogFlag) ajax(PATH+'HIOnline.y?cmd=onlog', {"auid":auid,"mid":mid,"amid":amid,"log":a});
}
/**********************************************************/
...
//	  });
//	if(seFlag==1){ //大于今天
//	    from.datepicker( "option", "minDate", '0D' );
//	    to.datepicker( "option", "minDate", '0D' );
//	}
//	if(seFlag==-1){ //小于今天
//	    from.datepicker( "option", "maxDate", '0D' );
//	    to.datepicker( "option", "maxDate", '0D' );
//	}
//	if(seDays && seDays>0){
//		
//	}
...
UI.msgBox = function(title, txt){
  layer.open({
    ...
    content: '<div class="f18 text-c pd50 dt5 color-focus"><br>'+title+'</div><div class="f14 pd50 dt10" style="line-height:35px;">'+txt+'<br><br></div>',
    area: ['400px']
  });
};
...
UI.shadeHtml = function(url,w,h,sfun){
  var iWidth = document.documentElement.clientWidth; 
  var iHeight = document.documentElement.clientHeight+document.documentElement.scrollTop*2;
  if(w==0 || w>iWidth-10) w = iWidth-30;
  if(h==0 || h>iHeight-10) h = iHeight-10;
  openBox = layer.open({
    title: '',
    type: 2, 
    content: url, //这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['http://sentsin.com', 'no']
    closeBtn: 0,
    scrollbar: false,
    shade: [0.6, '#000'], //如果你不想显示遮罩，可以shade: 0
    shadeClose: true,
    area: [w+'px', h+'px'],
    success: function(){
      parent.showShade();
...
util.UI.shadeHtml = UI.shadeHtml;
